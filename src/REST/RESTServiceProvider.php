...
final class RESTServiceProvider implements BootstrappableServiceProvider {
	...
	private function register_languages( Container $container ) {
		...
		$container->share( 'multilingualpress.rest.languages_update_arguments', function () {

			return new Endpoint\Languages\Update\EndpointArguments();
		} );

		$container->share( 'multilingualpress.rest.languages_update_handler', function ( Container $container ) {

			return new Endpoint\Languages\Update\RequestHandler(
				$container['multilingualpress.languages'],
				$container['multilingualpress.rest.languages_formatter'],
				$container['multilingualpress.rest.languages_schema'],
				$container['multilingualpress.rest_request_field_processor'],
	...
	private function register_site_relations( Container $container ) {
		...
		$container->share( 'multilingualpress.rest.site_relations_create_arguments', function ( Container $container ) {

			return new Endpoint\SiteRelations\Create\EndpointArguments(
				$container['multilingualpress.error_factory']
			);
		} );

		$container->share( 'multilingualpress.rest.site_relations_create_handler', function ( Container $container ) {

			return new Endpoint\SiteRelations\Create\RequestHandler(
	...
	private function add_languages_routes( Core\Route\Collection $route_collection, Container $container ) {
		...
		$route_collection->add( new Core\Route\Route(
			$base . '/(?P<id>\d+)',
			Core\Route\Options::from_arguments(
				$container['multilingualpress.rest.languages_update_handler'],
				$container['multilingualpress.rest.languages_update_arguments'],
				\WP_REST_Server::EDITABLE,
				[
					'permission_callback' => PermissionCallbackFactory::current_user_can( 'manage_sites' ),
				]
	...
	private function add_site_relations_routes( Core\Route\Collection $route_collection, Container $container ) {
		...
		$route_collection->add( new Core\Route\Route(
			$base,
			Core\Route\Options::from_arguments(
				$container['multilingualpress.rest.site_relations_create_handler'],
				$container['multilingualpress.rest.site_relations_create_arguments'],
				\WP_REST_Server::CREATABLE,
				[
					'permission_callback' => PermissionCallbackFactory::current_user_can( 'manage_sites' ),
				]
